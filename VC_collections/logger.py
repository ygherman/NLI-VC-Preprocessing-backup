...
def initialize_logger(branch, collection_id):
    ...
    reports_path = Path.cwd().parent.parent / ("VC-"+branch) / collection_id / "Data" / "reports"
    ...
    file_handler.setFormatter(logging.Formatter('[{levelname}] {asctime}s {name}  {message}', dt_fmt,
                                                style="{"))
    ...
    stream_handler.setFormatter(logging.Formatter('[{levelname}] {name}  {message}', style="{"))
